...
function InputBox() {
  ...
  const { generateVideo } = useContext(PolyMindContext);
  ...
  return (
    <div className="mt-10 w-full h-20 gap-4 p-10  flex items-center justify-center border-t border-t-[#1313ec50]">
      <input
        ...
        placeholder="Bir video oluşturmak için bir şeyler yazın..."
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        onKeyDown={(e) => e.key === "Enter" && handleSend()}
      />
      <button className="bg-[#1313ec] text-white justify-center flex items-center p-3 cursor-pointer rounded-full hover:opacity-80 transition-all duration-300">
